...
def create_mer_plot(plot_params):
    ...
    for n in range(len(snr_range)):
        for input_vector in input_vectors:
           ...
           # ext_hamming_decoded_vector = [int(b > 0.0) for b in ext_hamming_channel_vector]
            ...
            turbo_coded_errors[n] = turbo_coded_errors[n] + (turbo_coded_error_count > 0)
            hadamard_coded_errors[n]  = hadamard_coded_errors[n] + (hadamard_coded_error_count > 0)
            uncoded_errors[n] = uncoded_errors[n] + (uncoded_error_count > 0)

        print("Finished {} trials for SNR = {:8.2f} dB ...".format(num_trials, snr_range[n]))
    hadamard_mer_values = hadamard_coded_errors / (num_trials)
    turbo_coded_mer_values = turbo_coded_errors / (num_trials)
    uncoded_mer_values = uncoded_errors / (num_trials)

    def plot_data():
        plot.plot(snr_range, turbo_coded_mer_values, "r.-", label="Турбо код")
        plot.plot(snr_range, uncoded_mer_values, "b.-", label="Без код")
        plot.plot(snr_range, hadamard_mer_values, "g.-", label="Хадамардов код")

        plot.yscale("log")
        plot.title("Турбо код со рата на пренос 1/3, Информациски битови={}, Обиди={}".format(block_size, num_trials))
        plot.xlabel("SNR [dB]")
        plot.ylabel("Bit Error Rate (mer)")
        plot.grid(visible=True,which="major", linestyle="-")
        plot.grid(visible=True,which="minor", linestyle="--")
        plot.legend()
        plot.savefig("plot.mer.svg")
        plot.show()
    ...
    def plot_smooth_data():
        ...
        spl = make_interp_spline(snr_range, turbo_coded_mer_values)
        plot.plot(xnew, spl(xnew), "r.-", label="Турбо код")

        spl = make_interp_spline(snr_range, uncoded_mer_values)
        plot.plot(xnew, spl(xnew), "b.-", label="Без код")
        
        spl = make_interp_spline(snr_range, hadamard_mer_values)
        plot.plot(xnew, spl(xnew), "g.-", label="Хадамардов код")

        # plot.yscale("log")
        plot.title("Турбо код со рата на пренос 1/3, Информациски битови={}, Обиди={}".format(block_size, num_trials))
        plot.xlabel("SNR [dB]")
        plot.ylabel("Message Error Rate (MER)")
        plot.grid(visible=True,which="major", linestyle="-")
        plot.grid(visible=True,which="minor", linestyle="--")
        plot.legend()
        plot.savefig("plot.mer.smooth.svg")
        plot.show()
...
if __name__ == '__main__':
    ...
    create_mer_plot({'block_size':7,'num_trials':100,'snr':[-10,15,26]})